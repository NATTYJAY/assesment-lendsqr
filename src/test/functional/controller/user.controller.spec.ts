...
describe('User Controller', () => {
  ...
  let createUserApiUrl = '/api/v1/user';
  ...
  describe('Create', () => {
    it('should  create User with valid request body', async () => {
      ...
      let response = await chai.request(app).post(createUserApiUrl).type('application/json').send(params)
    ...
    it('should not create User with valid duplicate email in request body', async () => {
      ...
      let response1 = await chai.request(app).post(createUserApiUrl).type('application/json').send(params)
    ...
    it('should get all users ', async () => {
      const response = await chai.request(app).get(createUserApiUrl).type('application/json').send();
      expect(response.status).to.equal(200);
    ...
    it('should  create User with valid data and get using apis', async () => {
      ...
      let response1 = await chai.request(app).get(createUserApiUrl).type('application/json').send()
